/** @jsx html */
import { jsx, Router, h } from 'zaitun';
const html = jsx.html;
...
export default class Hero {
    ...
    view({ model, dispatch }) {
        return <div classNames="hero-container">
            <div classNames="page-container">
                {model.selected ? this.detailView({ movie: model.selected, dispatch }) : this.overviewView({ movies: model.movies, dispatch })}
            </div>
        </div>
    }
    fadeInOutStyle() {
        return { opacity: '0', delayed: { opacity: '1' }, remove: { opacity: '0' } }
    }   

    overviewView({ movies, dispatch }) {
        return h('div.page', { style: this.fadeInOutStyle }, [
            h('div.header', [
                h('div.header-content.overview', {
                    style: this.fadeInOutStyle,
                }, [
                        h('div.header-title', {
                            style: {
            ...
            h('div.page-content', [
                h('div.list', {
                    ...
                    h('div.row', {
                        on: { click: [dispatch, { type: 'select', payload: movie }] },
                    }, [
                            h('div.hero.rank', [
            ...
            h('div.header', [
                h('div.header-content.detail', {
                    style: { opacity: '1', remove: { opacity: '0' } },
                        ...
                        h('div.spacer'),
                        h('div.close-detail', {
                            on: { click: [dispatch, { type: 'select', payload: undefined }] },
                            style: {
                                transform: 'scale(0)',
                                delayed: { transform: 'scale(1)' },
                                destroy: { transform: 'scale(0)' }
