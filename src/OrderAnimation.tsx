...
export default class OrderAnimation {
    init() {
        return {
            sortBy: 'rank',
            totalHeight: 0,
    ...
    view({ model, dispatch }) {
        const data = model.data.reduce((acc, m) => {           
            var last = acc[acc.length - 1];
            m.offset = last ? last.offset + last.elmHeight + model.margin : model.margin;
            return acc.concat(m);
        }, []);
        model.totalHeight = data[data.length - 1].offset + data[data.length - 1].elmHeight;
        
        return <div classNames="oa">
            <h1>Top 10 movies</h1>
            <div>
                <a classNames="btn add" on-click={[dispatch, { type: 'add', dispatch:dispatch }]}>Add</a>
                <span>Sort by: </span>
                <span classNames="btn-group">
                    <a classNames="btn rank" on-click={[dispatch,{type:'sort', payload:'rank'}]} class={{ active: model.sortBy === 'rank' }}>Rank</a>
                    <a classNames="btn title" on-click={[dispatch,{type:'sort', payload:'title'}]} class={{ active: model.sortBy === 'title' }}>Title</a>
                    <a classNames="btn desc" on-click={[dispatch,{type:'sort', payload:'desc'}]} class={{ active: model.sortBy === 'desc' }}>Description</a>
                </span>
            </div>
            <div classNames="list" style={{ height: model.totalHeight + 'px' }}>
                {data.map(item => <this.MovieView  key={item.rank} model={item} dispatch={dispatch} />)}
            </div>
        </div>
    }
    MovieView({ model, dispatch }) {       
       
        return <div classNames="row"
            style={{
                opacity: '0', transform: 'translate(-200px)',
                delayed: { transform: `translateY(${model.offset}px)`, opacity: '1' },
                remove: { opacity: '0', transform: `translateY(${model.offset}px) translateX(200px)` }
            }}
            hook={{ insert: vnode => { model.elmHeight = vnode.elm.offsetHeight; } }}
        >
            <div style={{ fontWeight: 'bold' }}>{model.rank}</div>
            <div>{model.title}</div>
            <div>{model.desc}</div>
            <div on-click={[dispatch, { type: 'remove', payload: model.rank }]} classNames="btn rm-btn">x</div>
        </div>
    }
    update(model, action) {
        switch (action.type) {
            case 'add':
                var n = model.data[0];
                model.data = [{ rank: model.data.length+1, title: n.title, desc: n.desc, elmHeight: 0 }].concat(model.data);
                setTimeout(function() {
                   action.dispatch({}); 
                }, 0);
                return model;
            case 'remove':
                model.data = model.data.filter(_ => _.rank !== action.payload);               
                return { ...model };
            case 'sort':
